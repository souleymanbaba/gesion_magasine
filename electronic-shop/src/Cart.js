...
//           `http://localhost:8080/api/customer/cart/${userId}`
//         );
...
//       setOrderPlaced(true);
//       setShowModal(false);
//       // Rafraîchir les données du panier après la commande
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setErrorMessage(t('en.Error placing order. Please try again.'));
      }
    } catch (error) {
      console.error('Error placing order:', error);
      setErrorMessage(t('en.Error placing order. Please try again.'));
